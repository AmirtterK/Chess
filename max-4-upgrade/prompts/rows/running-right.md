Create one horizontal animation strip for Codex pet `max-4-v2`, state `running-right`.
...
Identity: same pet in every frame: Upgrade the existing racing-driver pet while preserving its closed helmet, navy suit, red-white-yellow helmet palette, and pixel-art silhouette; add coherent clockwise looking directions using head and visor orientation.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Preserve the established pixel-art sprite style, compact full-body silhouette, helmet visor and racing suit details..
