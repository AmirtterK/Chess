Create Codex pet row `running-right` for `max-4-v2`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Upgrade the existing racing-driver pet while preserving its closed helmet, navy suit, red-white-yellow helmet palette, and pixel-art silhouette; add coherent clockwise looking directions using head and visor orientation.. Preserve silhouette, face, palette, material, proportions, markings, and props.
