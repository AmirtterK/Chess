Create Codex pet row `running-right` for `dyt`: exactly 8 full-body frames in one horizontal strip on flat pure blue #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing cute chibi anime girl identity, long brown hair with white bow, blue cardigan, cream skirt, and white shoes.. Preserve silhouette, face, palette, material, proportions, markings, and props.
